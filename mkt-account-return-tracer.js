...
function beginAccountWindow(reason) {
  accountWindowUntil = Date.now() + ACCOUNT_WINDOW_MS;
  emit("account-window", { reason, durationMs: ACCOUNT_WINDOW_MS });
}
function accountWindowActive() {
  return Date.now() <= accountWindowUntil;
}
...
function sanitizeIntent(intent) {
  const result = {};
  try {
    result.action = safeString(intent.getAction());
  } catch (_) {
  }
  try {
    result.component = safeString(intent.getComponent());
  } catch (_) {
  }
  try {
    ...
    if (uri !== null) {
      result.uri = {
        scheme: safeString(uri.getScheme()),
        host: safeString(uri.getHost()),
        path: safeString(uri.getPath()),
        queryKeys: Array.from(uri.getQueryParameterNames().toArray()).map(safeString),
        hasFragment: uri.getFragment() !== null
      };
    }
  } catch (error) {
    result.uriError = safeString(error);
  }
  try {
    const extras = intent.getExtras();
    result.extraKeys = extras === null ? [] : Array.from(extras.keySet().toArray()).map(safeString);
  } catch (error) {
    result.extrasError = safeString(error);
  }
  return result;
}
function installJavaHooks() {
  const tryInstall = () => {
    ...
    frida_java_bridge_default.perform(() => {
      try {
        const Instrumentation = frida_java_bridge_default.use("android.app.Instrumentation");
        const newIntent = Instrumentation.callActivityOnNewIntent.overload("android.app.Activity", "android.content.Intent");
        newIntent.implementation = function(activity, intent) {
          beginAccountWindow("onNewIntent");
          emit("android-intent", {
            activity: safeString(activity.getClass().getName()),
            intent: sanitizeIntent(intent)
          });
          return newIntent.call(this, activity, intent);
        };
        const resume = Instrumentation.callActivityOnResume.overload("android.app.Activity");
        resume.implementation = function(activity) {
          const name = safeString(activity.getClass().getName());
          beginAccountWindow("onResume");
      ...
      try {
        ...
        for (const methodName of ["clear", "apply", "commit"]) {
          const dispatcher = Editor[methodName];
          if (dispatcher === void 0)
            continue;
          for (const overload of dispatcher.overloads) {
        ...
        emit("java-hook-error", { area: "preferences", error: safeString(error) });
      }
      try {
        const FOS = frida_java_bridge_default.use("java.io.FileOutputStream");
        for (const overload of FOS.$init.overloads) {
          overload.implementation = function(...args) {
        ...
        emit("java-hook-error", { area: "files", error: safeString(error) });
      }
      emit("java-hooks-ready", { message: "Sanitized intent, lifecycle, preference, and file hooks installed." });
    });
  };
  tryInstall();
}
function installNativePathHooks() {
  try {
    const libc = Process.getModuleByName("libc.so");
    for (const spec of [
      { name: "open", pathIndex: 0 },
      { name: "openat", pathIndex: 1 },
      { name: "fopen", pathIndex: 0 },
      { name: "rename", pathIndex: 0 },
      { name: "unlink", pathIndex: 0 }
    ]) {
      const address = libc.findExportByName(spec.name);
      if (address === null)
        continue;
    ...
    emit("native-file-hooks-ready");
  } catch (error) {
    emit("native-hook-error", { error: safeString(error) });
  }
}
function installIl2CppHooks() {
  Il2Cpp.perform(() => {
    const classPattern = /^(Shelp\.(NintendoAccount|LoginResult)|Sks\.NintendoAccount|Game\.(SakashoNintendoAccountManager|SakashoUtil\.NintendoAccount|SakashoInnerUtil\.NintendoAccount|AuthorizeResultHolder))/;
    const methodPattern = /(account|authorize|resume|link|success|error|result|region|token|switch|copy)/i;
    const hooked = /* @__PURE__ */ new Set();
    const catalog = [];
    for (const assembly of Il2Cpp.domain.assemblies) {
      for (const klass of assembly.image.classes) {
        const className = klass.type.name;
        if (!classPattern.test(className))
          continue;
        const methods = klass.methods.filter((method) => methodPattern.test(method.name));
        if (methods.length === 0)
          continue;
        catalog.push({
          class: className,
          methods: methods.map((method) => `${method.name}(${method.parameters.map((p) => p.type.name).join(", ")})`)
        });
        for (const method of methods) {
          if (hooked.size >= MAX_IL2CPP_HOOKS || method.virtualAddress.isNull())
            continue;
          const address = method.virtualAddress.toString();
          if (hooked.has(address))
            continue;
          hooked.add(address);
          Interceptor.attach(method.virtualAddress, {
            onEnter() {
              ...
              emit("il2cpp-account-call", {
                class: className,
                method: method.name,
                parameters: method.parameters.map((p) => p.type.name),
    ...
    emit("il2cpp-account-hooks-ready", {
      hookCount: hooked.size,
      candidateClasses: catalog.slice(0, 100)
    });
  });
}
emit("account-return-tracer-start", { message: "All sensitive intent and preference values are redacted." });
installNativePathHooks();
installJavaHooks();
installIl2CppHooks();
