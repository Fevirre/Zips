...
function classChain(start) {
  const result = [];
  let current = start;
  while (current !== null && result.length < 16) {
    result.push(current);
  ...
  for (const klass of classChain(object.class)) {
    for (const field of klass.fields) {
      ...
      try {
        ...
        fields[`${klass.name}.${field.name}`] = `<error: ${safeText(error)}>`;
      }
    }
  }
...
function describePointer(address) {
  ...
  return symbol.name ? `${symbol.moduleName}!${symbol.name}` : address.toString();
}
function installButtonTrace() {
  const assembly = Il2Cpp.domain.tryAssembly("UnityEngine.UI");
  if (assembly === null) {
    send({ type: "account-trace-status", message: "UnityEngine.UI is not loaded yet" });
    return;
  }
  const buttonClass = assembly.image.tryClass("UnityEngine.UI.Button");
  if (buttonClass === null) {
    send({ type: "account-trace-error", message: "UnityEngine.UI.Button was not found" });
    return;
  }
  const press = buttonClass.methods.find((method) => method.name === "Press" && method.parameterCount === 0);
  if (press === void 0 || press.virtualAddress.isNull()) {
    send({ type: "account-trace-error", message: "Button.Press() was not found" });
    return;
  }
  Interceptor.attach(press.virtualAddress, {
    onEnter(args) {
      if (!armed || captured)
        return;
      captured = true;
...
Il2Cpp.perform(() => installButtonTrace());
