  ...
  "index.ts"(exports) {
    ...
    function installHooks() {
      frida_java_bridge_1.default.perform(() => {
        ...
        try {
          const getPackageInfoFlags = ApplicationPackageManager.getPackageInfo.overload("java.lang.String", "android.content.pm.PackageManager$PackageInfoFlags");
          getPackageInfoFlags.implementation = function(packageName, flags) {
            const info = getPackageInfoFlags.call(this, packageName, flags);
            if (isTarget(packageName)) {
        ...
        const getApkContentsSigners = SigningInfo.getApkContentsSigners.overload();
        getApkContentsSigners.implementation = function() {
          if (trackedSigningInfo.has(this.hashCode().toString())) {
            console.log("[mkt-sigspoof] spoofed APK content signers");
            return originalSignatures;
          }
          return getApkContentsSigners.call(this);
        };
        const getSigningCertificateHistory = SigningInfo.getSigningCertificateHistory.overload();
        getSigningCertificateHistory.implementation = function() {
          if (trackedSigningInfo.has(this.hashCode().toString())) {
            console.log("[mkt-sigspoof] spoofed signing certificate history");
            return originalSignatures;
          }
          return getSigningCertificateHistory.call(this);
        };
        for (const overload of ApplicationPackageManager.hasSigningCertificate.overloads) {
          overload.implementation = function(packageName, certificate, type) {
            if (isTarget(packageName)) {
              console.log(`[mkt-sigspoof] hasSigningCertificate(${packageName}) => true`);
              return true;
            }
            return overload.call(this, packageName, certificate, type);
          };
        }
        try {
          const checkSignatures = ApplicationPackageManager.checkSignatures.overload("java.lang.String", "java.lang.String");
          checkSignatures.implementation = function(first, second) {
            if (isTarget(first) || isTarget(second)) {
              console.log(`[mkt-sigspoof] checkSignatures(${first}, ${second}) => MATCH`);
              return PackageManager.SIGNATURE_MATCH.value;
            }
            return checkSignatures.call(this, first, second);
          };
        } catch (_) {
        }
