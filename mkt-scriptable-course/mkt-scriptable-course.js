...
// mkt-scriptable-course.ts
var require_mkt_scriptable_course = __commonJS({
  "mkt-scriptable-course.ts"(exports) {
    init_node_globals();
    Object.defineProperty(exports, "__esModule", { value: true });
    init_dist();
    var armed = false;
    var scanning = false;
    var dumped = /* @__PURE__ */ new Set();
    var MAX_DEPTH = 12;
    var MAX_ARRAY = 5e4;
    var MAX_NODES = 5e4;
    ...
    function classDescription(klass) {
      ...
      for (const current of klass.hierarchy({ includeCurrent: true })) {
        hierarchy.push({
          ...
          fields: current.fields.filter((field) => !field.isStatic).map((field) => ({ name: field.name, type: field.type.name, offset: field.offset }))
        });
      }
      return hierarchy;
    }
    function reflectObject(root) {
      ...
      function visit(value, depth) {
        ...
        if (value instanceof Il2Cpp.Array) {
          ...
          const items = [];
          for (let i = 0; i < length; i++)
            items.push(visit(value.get(i), depth + 1));
          if (value.length > length)
            items.push(`<${value.length - length} more elements>`);
          return items;
        }
        ...
        if (value instanceof Il2Cpp.Object) {
          ...
          if (seen.has(handle))
            return { "$ref": handle, "$type": value.class.fullName };
          seen.add(handle);
          const result = {
          ...
          for (const klass of value.class.hierarchy({ includeCurrent: true })) {
            for (const field of klass.fields) {
              if (field.isStatic || Object.prototype.hasOwnProperty.call(result, field.name))
                continue;
              try {
                result[field.name] = visit(value.field(field.name).value, depth + 1);
              } catch (error) {
    ...
    function findScriptableCourseClasses() {
      const matches = [];
      for (const assembly of Il2Cpp.domain.assemblies) {
        for (const klass of assembly.image.classes) {
          const lower = klass.name.toLowerCase();
          if (lower === "scriptablecourse" || lower.includes("scriptablecourse"))
            matches.push(klass);
        }
      }
      return matches;
...
export default require_mkt_scriptable_course();
